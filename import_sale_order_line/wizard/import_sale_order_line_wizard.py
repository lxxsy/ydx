...
class import_sale_order_line_wizard(models.TransientModel):
    _name = "import.sale.order.line.wizard"
    _description = u"导入订单明细"

    master_id = fields.Many2one('sale.order', u'订单编号', ondelete="cascade", readonly=True)
    ...
    def default_get(self, fields):
        if self._context is None:
            self._context = {}
        res = super(import_sale_order_line_wizard, self).default_get(fields)
    ...
    def action_confirm(self):
        ...
        if self.excel_file:
            ...
            for row in range(1, sh.nrows):  # 第一行为caption
                ...
                obj = self.env['sale.order.line'].create(item)
                obj.product_id_change()
