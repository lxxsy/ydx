...
class AccountAnalyticLine(models.Model):
    ...
    sub_sale_order_line = fields.Many2one('sub.sale.order', string='Sales Order Item', domain=lambda self: self._default_sale_line_domain())

    @api.model
    def create(self, values):
        result = super(AccountAnalyticLine, self).create(values)
        if 'sub_sale_order_line' not in values and not result.sub_sale_order_line and result.product_id and result.product_id.expense_policy not in [False, 'no'] and result.amount <= 0:  # allow to force a False value for so_line
    ...
    def write(self, values):
        ...
        if 'sub_sale_order_line' not in values:  # allow to force a False value for so_line
            # only take the AAL from expense or vendor bill, meaning having a negative amount
            self.sudo().filtered(lambda aal: not aal.sub_sale_order_line and aal.product_id and aal.product_id.expense_policy not in [False, 'no'] and aal.amount <= 0)._sale_determine_order_line()
        return result
    ...
    def _sale_prepare_sale_order_line_values(self, order, price):
        self.ensure_one()
        last_sub_sale_order_line = self.env['sub.sale.order'].search([('order_id', '=', order.id)], order='sequence desc', limit=1)
        last_sequence = last_sub_sale_order_line.sequence + 1 if last_sub_sale_order_line else 100
    ...
    def _sale_determine_order_line(self):
        ...
        for analytic_line in self:
            ...
            if analytic_line.product_id.expense_policy == 'sales_price' and analytic_line.product_id.invoice_policy == 'delivery':
                sub_sale_order_line = self.env['sub.sale.order'].search([
            ...
            if not sub_sale_order_line:
                # generate a new SO line
                sub_sale_order_line_values = analytic_line._sale_prepare_sale_order_line_values(sale_order, price)
                sub_sale_order_line = self.env['sub.sale.order'].create(sub_sale_order_line_values)
                sub_sale_order_line._compute_tax_id()

            if sub_sale_order_line:  # if so line found or created, then update AAL (this will trigger the recomputation of qty delivered on SO line)
                value_to_write.setdefault(sub_sale_order_line.id, self.env['account.analytic.line'])
                value_to_write[sub_sale_order_line.id] |= analytic_line

        # write so line on (maybe) multiple AAL to trigger only one read_group per SO line
        for sub_sale_order_line_id, analytic_lines in value_to_write.items():
            if analytic_lines:
                analytic_lines.write({'sub_sale_order_line': sub_sale_order_line_id})
