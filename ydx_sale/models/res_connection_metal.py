...
class ResConnectionMetal(models.Model):
    _inherit = 'res.connection.metal'
    ...
    order_id = fields.Many2one('sale.order', related='sub_order_id.order_id', string='Order Reference', required=True, index=True, copy=False)
    sub_order_id = fields.Many2one('sub.sale.order', string='Sub Sale Order', required=True, ondelete='cascade', index=True, copy=False)
    move_ids = fields.One2many('stock.move', 'sale_connection_metal_id', string='Stock Moves')
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        values.update({
            ...
            'group_id': group_id,
            'sale_connection_metal_id': self.id,
