...
class SaleReturnAdvancePaymentInv(models.TransientModel):
    _name = "sale.return.advance.payment.inv"
    _description = "Sales Return Advance Payment Invoice"
    ...
    def _create_invoice(self, order, so_line, amount):
        ...
        invoice = inv_obj.create({
            ...
            'invoice_line_ids': [(0, 0, {
                ...
                'sale_return_line_ids': [(6, 0, [so_line.id])],
    ...
    def create_invoices(self):
        sale_returns = self.env['sale.return'].browse(self._context.get('active_ids', []))

        if self.advance_payment_method == 'delivered':
            sale_returns.action_invoice_create()
        elif self.advance_payment_method == 'all':
            sale_returns.action_invoice_create(final=True)
        else:
            ...
            sale_return_line_obj = self.env['sale.return.line']
            for order in sale_returns:
                if self.advance_payment_method == 'percentage':
                    amount = order.amount_untaxed * self.amount / 100
                else:
                    amount = self.amount
                ...
                so_line = sale_return_line_obj.create({
        ...
        if self._context.get('open_invoices', False):
            return sale_returns.action_view_invoice()
...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def _create_invoice(self, order, so_line, amount):
        ...
        context = {'lang': order.partner_id.lang}
        if self.advance_payment_method == 'percentage':
            amount = order.sub_sale_amount_untaxed * self.amount / 100
            name = _("Down payment of %s%%") % (self.amount,)
        else:
            amount = self.amount
            name = _('Down Payment')
        ...
        invoice = inv_obj.create({
            ...
            'invoice_line_ids': [(0, 0, {
                ...
                'uom_id': self.product_id.uom_id.id,
                'product_id': self.product_id.id,
                'sub_sale_line_ids': [(6, 0, [so_line.id])],
                'invoice_line_tax_ids': [(6, 0, tax_ids)],
                'analytic_tag_ids': [(6, 0, so_line.analytic_tag_ids.ids)],
                'account_analytic_id': order.analytic_account_id.id or False,
            })],
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'delivered':
            ...
            sale_line_obj = self.env['sub.sale.order']
            for order in sale_orders:
                ...
                so_line = sale_line_obj.create({
                    'name': _('Advance: %s') % (time.strftime(' %Y %m %d %H:%M'),),
                    'price_unit': amount,
                    'product_uom_qty':0.0,
                    'order_id': order.id,
                    'discount': 0.0,
