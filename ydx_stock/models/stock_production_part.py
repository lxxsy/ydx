...
class StockProductionPart(models.Model):
    ...
    product_id = fields.Many2one('product.product', string='Product', domain=[('sale_ok', '=', True), ('fuction_type', '=', 'parts')], change_default=True, ondelete='restrict')
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('stock.connection.metal') or '/'
        return super(StockProductionPart, self).create(vals)
